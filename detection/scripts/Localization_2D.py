...
class Localization:
    def __init__(self):
        ...
        self.camerapose = [0, 0, 0] ##Caemra pose for transform

        self.det_points = np.array([["class_id", "class_name", "x", "y", "sumx", "sumy", "num"]])

        ##Subscriber for SLAM and Detection data
        self.subpose = rospy.Subscriber('/slam_out_pose', PoseStamped, self.poseCallback, queue_size=1) ##Hector slam:/slam_out_pose, Cartographer:/tracked_pose
        ...
        ##self.file = open('/home/rapha/catkin_ws/src/detection/scripts/output/output.txt', 'w') ##For txt output
    ...
    def localization(self):
        ...
        if self.det_points.shape[0] > 1:
            for i in range(1, self.det_points.shape[0]):
                ...
                ##self.file.write('id : ' + self.det_points[i][0] + ', class : ' + self.det_points[i][1] + ', posx : ' + self.det_points[i][2] + ', posy : ' + self.det_points[i][3] + '\n') ##For txt output
        ##self.file.write('======================================================' + '\n') ##For txt output
